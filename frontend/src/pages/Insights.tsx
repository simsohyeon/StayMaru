...
/**
 * 경북 데이터 인사이트 (/insights) — 방문자 버블 지도 + 한적한 순 랭킹 + 함께 찾은 곳.
 * 같은 실방문자 데이터를 지도(비례 심볼)와 목록(바) 두 형태로 보여준다 — 지도는 접근성이
 * 약해 목록이 동등한 대안이 된다.
 * 데이터가 없으면 가짜로 채우지 않고 안내를 띄우며, 코스 엔진은 정적 hiddenBoost 로 계속 동작한다.
 */
