...
set_cell(t.cell(2, 1),
         '경상북도 22개 시군의 전통문화 여행지를 한 곳에서 찾고, 챗봇 대화 몇 번으로 '
...
set_cell(tables(2)[0].cell(0, 1), [
    ...
    '3. 맞춤형 코스 자동 생성 — 챗봇에 지역·기간·동반자·취향만 답하면 이동 동선과 날씨를 반영한 일차별 일정을 자동 구성',
    '4. 코스 편집·지도·공유 — 순서 변경·장소 추가/삭제 후 동선 재최적화, 지도로 동선 확인, 찜·카카오톡 공유·방 코드 공동 편집',
    '5. 숨은 경북 발견 — 시군별 방문자 통계로 계산한 한적지수로 봉화·청송·영양 등 덜 알려진 지역을 우선 추천하고 데이터 인사이트 제공',
...
set_cell(t.cell(1, 1), [
    '[주요 화면 이미지 4개 삽입]',
    '① 홈 — 챗봇 코스 만들기   ② 탐색 — 카테고리·지도 결과',
    '③ 코스 결과 — 일차별 일정표   ④ 코스 지도 — 방문 순서·동선',
])
...
FEATURES = [
    (5, '경북 전통문화 통합 탐색',
     ...
     ['1. 탐색 화면에서 경북 시군(거점)을 고른다.',
      '2. 카테고리(숙박·체험·관광·전통시장·축제)를 선택한다.',
      '3. 경북 특화 키워드로 검색하고 정렬·필터를 적용한다.',
      '4. 지도와 목록으로 결과를 확인한다.']),
    (6, '상세 정보 · 예약 링크 원스톱 연결',
     '관광공사 API의 소개·운영정보·연락처·홈페이지를 한 화면에 모아 예약·문의 채널까지 바로 연결하고, '
     '‘함께 찾은 곳’ 연관 추천으로 다음 장소를 이어 주는 기능입니다.',
     ['1. 탐색 결과에서 장소를 선택한다.',
      '2. 대표 이미지·소개·운영정보를 확인한다.',
      '3. 연락처·홈페이지·예약 링크와 연관 추천을 본다.',
      '4. 예약·문의로 바로 이동하거나 찜한다.']),
    (7, '여행 코스 자동 구성',
     '챗봇 답변만으로 장소를 점수화해 고르고, 이동거리가 최소가 되도록 일차별 일정을 짜는 기능입니다. '
     '한 생활권으로 동선을 묶어 장거리 이동을 막고, 비 예보일은 실내 위주로 구성하며, 찜한 장소와 여행 기간의 축제를 함께 반영합니다.',
     ['1. 홈 챗봇에서 ‘코스 만들기’를 시작한다.',
      '2. 지역·기간·동반자·취향을 답한다.',
      '3. 날씨·축제·동선을 반영한 코스가 자동 생성된다.',
      '4. 순서·장소를 바꾸면 동선이 재최적화된다.']),
    (8, '지도 동선 · 찜 · 공유',
     '코스를 지도 위 방문 순서와 구간 거리로 보여 주고, 찜·카카오톡 공유·방 코드 기반 실시간 공동 편집으로 '
     '친구·가족과 함께 여행을 계획하는 기능입니다.',
     ['1. 코스 결과에서 지도 보기를 연다.',
      '2. 방문 순서 마커·동선·구간 거리를 확인한다.',
      '3. 코스를 찜하거나 카카오톡으로 공유한다.',
      '4. 방 코드로 친구와 같은 코스를 함께 편집한다.']),
    (9, '숨은 경북 · 데이터 인사이트',
     '관광 데이터랩의 시군별 방문자 통계로 한적지수를 계산해 방문이 적은 내륙 시군의 전통문화 자원을 우선 추천하고, '
     '시군별 방문 통계를 인사이트 화면으로 보여 주는 기능입니다.',
     ['1. 코스 취향에서 ‘숨은 경북’을 선택한다.',
      '2. 한적지수가 높은 시군 자원이 우선 추천된다.',
      '3. 인사이트 화면에서 시군별 방문 통계를 확인한다.',
      '4. 추천된 숨은 장소를 코스에 담는다.']),
]
FLOW_SHIFT = 446674      # 기능 설명이 3줄로 늘어도 흐름도 표와 겹치지 않도록 아래로 이동
...
API_1 = [
    ...
    ('한국관광공사 국문 관광정보 서비스 (KorService2) — 키워드 검색·행사정보 조회',
     '서원·고택·탈춤·가야·신라 등 경북 특화 키워드로 전통문화 자원을 검색하고, 여행 기간에 열리는 경북 축제·행사를 코스에 자동 포함'),
    ('한국관광공사 국문 관광정보 서비스 (KorService2) — 공통·소개·반복·이미지 정보 조회',
...
API_2 = [
    ...
    ('한국관광공사 반려동물 동반여행 서비스 (KorPetTourService)',
     '반려동물 동반 가능 여부·동반 조건을 조회해 입장 가능한 장소 위주로 코스를 구성'),
    ('한국관광공사 관광지 연관 추천 정보 서비스 (TarRlteService · 관광 빅데이터)',
     '실제 방문자의 동반 방문 패턴으로 장소 상세 화면에 ‘함께 찾은 곳’을 추천하고 코스 후보를 확장'),
    ('한국관광공사 한국관광 데이터랩 (DataLabService · 관광 빅데이터)',
     '경북 시군별 방문자 통계로 ‘한적지수’를 계산해 방문이 적은 시군에 추천 가중치를 부여하고, 인사이트 화면의 시군별 랭킹을 구성'),
...
ETC = [
    ('행정안전부 전국문화축제 표준데이터 (공공데이터포털 OpenAPI)',
     '경북 축제의 일정·장소·주최 정보를 보강해 축제 캘린더를 구성하고, 여행 기간과 겹치는 축제만 코스에 연계'),
    ('기상청 단기예보 조회서비스 (공공데이터포털 OpenAPI)',
     '여행 날짜의 강수확률(POP)을 조회해 비 예보일에는 실내 장소를, 맑은 날에는 실외 장소를 우선 배치 (예보 범위를 벗어나면 평년 강수 경향으로 대체)'),
    ('카카오맵 JavaScript SDK · 카카오톡 공유 SDK',
     '코스의 방문 순서와 이동 동선을 지도에 표시하고, 완성한 코스를 카카오톡으로 공유'),
    ('한국불교문화사업단 템플스테이 포털 (templestay.com) 사찰 정보',
     '경북 템플스테이 운영 사찰 목록을 연계해 전통 숙박 카테고리를 보강'),
]
...
set_cell(last.cell(0, 1), [
    ...
    '• 완성형 코스 — 목록 나열이 아니라 이동거리를 최소화한 일차별 일정을 자동 구성 (비 오는 날 실내 / 맑은 날 실외)',
    '• 경북 특화 — 시군구 코드·경북 문화 키워드로 전통문화 자원만 선별, 방문이 적은 내륙 시군을 우선 노출해 관광 쏠림 완화',
    '• 함께 계획 — 로그인 없이 방 코드 하나로 친구·가족과 실시간 공동 편집, 카카오톡 공유',
