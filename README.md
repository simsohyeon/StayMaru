...
- **코스 자동 생성 엔진** — 카테고리 가중치 × 거점 반경 감점 × **숨은지역 보너스(데이터랩 실방문자 기반,
  미구독 시 정적 지수 폴백)** × 찜 가중치 × **월별 평년 강수 경향(기상청 평년값 근사)** 가중치를 결합해
  점수화 → 카테고리 다양성 보장 → 최근접 클러스터 동선 정렬.
  (강수는 **기상청 단기예보(POP) 실연동** — 시군 격자좌표로 호출, 예보범위(약 3일) 밖이거나 미신청 시 평년값으로 graceful 폴백하며 출처를 "단기예보/평년"으로 구분 표기.)
